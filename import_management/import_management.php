    ...
    <main class="main-content">
      <header class="page-header">
        ...
        <div class="header-actions">
          <button id="openReportModal" class="btn btn-secondary">
        ...
        <div class="filter-dropdowns">
          ...
          <div class="filter-group filter-date-group">
            <div class="date-label-refresh">
              ...
              <button type="button" id="refreshBtn" title="Reset Filters"><i class="fas fa-sync-alt"></i></button>
            </div>
          ...
          <tbody>
            <?php
              ...
              while ($row = $res->fetch_assoc()) {
                ...
                $dateClass = ($arrivalDate <= $today) ? "date-done" : "date-pending";
                echo "<tr>
                        ...
                        <td class='actions'>
                          <form method='GET' action='import_management.php' style='display:inline;'>
                            ...
                            <button type='submit' class='action-btn edit' title='Edit'><i class='fa-solid fa-pen'></i></button>
                          </form>
                          <form method='POST' action='save_import.php' style='display:inline;' onsubmit='return confirm(\"Are you sure you want to delete this import?\");'>
                            ...
                            <button type='submit' class='action-btn delete' title='Delete'><i class='fa-regular fa-trash-can fa-lg'></i></button>
                          </form>
                        </td>
                      </tr>";
              }
    ...
    <div class="modal-content">
      ...
      <form id="addImportForm" method="POST" action="save_import.php">
        ...
        <select name="product_id" id="productSelect" required>
          ...
          <?php
            $products = $conn->query("SELECT p.product_id, p.product_name, p.price_per_kg, s.supplier_id FROM products p LEFT JOIN supplier s ON p.product_id = s.s_productid");
            while ($p = $products->fetch_assoc()) {
              echo "<option value='{$p['product_id']}' data-supplier='{$p['supplier_id']}' data-name='{$p['product_name']}' data-price='{$p['price_per_kg']}'>{$p['product_name']}</option>";
            }
          ?>
        </select>
